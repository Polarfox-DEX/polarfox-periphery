...
describe("ExampleSlidingWindowOracle", () => {
  ...
  beforeEach(`set start time to ${startTime}`, () =>
    mineBlock(provider, startTime)
  );
  ...
  it("requires windowSize to be evenly divisible by granularity", async () => {
    await expect(
      deployOracle(defaultWindowSize - 1, defaultGranularity)
    ).to.be.revertedWith("SlidingWindowOracle: WINDOW_NOT_EVENLY_DIVISIBLE");
  });
  ...
  it("computes the periodSize correctly", async () => {
    ...
    const oracleOther = await deployOracle(
      defaultWindowSize * 2,
  ...
  describe("#update", () => {
    ...
    beforeEach(
      "deploy oracle",
      async () =>
        (slidingWindowOracle = await deployOracle(
          defaultWindowSize,
          defaultGranularity
        ))
    );
    ...
    it("succeeds", async () => {
      await slidingWindowOracle.update(
        token0.address,
        token1.address,
        overrides
      );
    ...
    }).retries(100); // we may have slight differences between pair blockTimestamp and the expected timestamp
    ...
    it("gas for first update (allocates empty array)", async () => {
      const tx = await slidingWindowOracle.update(
        token0.address,
        token1.address,
        overrides
      );
    ...
    it("fails for invalid pair", async () => {
      await expect(slidingWindowOracle.update(wavax.address, token1.address)).to
        .be.reverted;
    });
  });
  ...
  describe("#consult", () => {
    ...
    it("fails for invalid pair", async () => {
      await expect(
        slidingWindowOracle.consult(wavax.address, 0, token1.address)
      ).to.be.reverted;
    });
    ...
    describe("happy path", () => {
      ...
      }).retries(10); // test flaky because timestamps aren't mocked
    ...
    describe("price changes over period", () => {
      ...
      describe("hour 32", () => {
        beforeEach("set hour 32", () =>
          mineBlock(provider, startTime + 32 * hour)
        );
        it("provides the correct ratio in consult token0", async () => {
          // at hour 23, price of token 0 spent 3 hours at 2, 3 hours at 1, 17 hours at 0.5 so price should
          // be less than 1
          expect(
            await slidingWindowOracle.consult(
              token0.address,
              100,
              token1.address
            )
